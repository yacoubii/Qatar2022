...
export class SpectatorComponent implements OnInit{
  ...
  constructor(private spectatorService: SpectatorService, private ticketService: TicketService,private tokenStorageService:TokenStorageService,private router: Router) { }
  ...
  /*public onAddSpectator(addForm: NgForm):void{
    ...
    this.spectatorService.addSpectator(this.myObject).subscribe(
      async (response: any) => {
        ...
        this.spectatorService.linkSpectatorToTicket(this.spectatorId,this.ticketId.tickets[0]).subscribe(
          (response: any) => {
            console.log(response)
          }, (error: HttpErrorResponse) => {
